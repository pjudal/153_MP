...
int main(int argc, char *argv[])
{
  ...
  std::cout << "Guide: Enter input WITHOUT space. e.g. 3x^2 + 1 = 301\n";
  std::cout << "Please enter A(x): ";
  getline (std::cin, a);
  std::cout << "Please enter B(x): ";
  getline (std::cin, b);
  std::cout << "Please enter P(x) (binary): ";
  getline (std::cin, p);

  if (!((a.find_first_not_of("0123456789") == std::string::npos) && (b.find_first_not_of("0123456789") == std::string::npos))) {
    printf("\nBoth A(x) and B(x) must be in integer form representing their own polynomials.\nPlease try again.\n\n");
  ...
  int a_fin[1024], b_fin[1024];
  unsigned int p_fin[1024];
  int a_index=0, b_index=0, p_index=0;
  char temp[1024];
  strncpy(temp, a.c_str(), sizeof(temp));
  temp[sizeof(temp) - 1] = 0;
  for (i=0; i < sizeof(temp); i++) {
    if (temp[i] != 0) a_index++;
    int new_int = temp[i] - '0';
    a_fin[i] = new_int;
  } 


  temp[0] = 0;
  strncpy(temp, b.c_str(), sizeof(temp));
  temp[sizeof(temp) - 1] = 0;
  ...
  temp[0] = 0;
  strncpy(temp, p.c_str(), sizeof(temp));
  temp[sizeof(temp) - 1] = 0;
  
  for (i=0; i < sizeof(temp); i++) {
    if (temp[i] != 0) p_index++;
    unsigned int new_int = temp[i] - '0';
    p_fin[i] = new_int;
  }

  unsigned int poly[p_index];
  int ctr=0;
  for (i=p_index-1; i >= 0; i--) {
    poly[ctr] = p_fin[i];
    ctr++;
  }
  GaloisField gf(p_index-1,poly);


  GaloisFieldElement gfe1[a_index];
  GaloisFieldElement gfe2[b_index];

  ctr=0;
  for (i=a_index-1; i >= 0; i--) {
    gfe1[ctr] = GaloisFieldElement(&gf, a_fin[i]);
    ctr++;
  }

  ctr=0;
  for (i=b_index-1; i >= 0; i--) {
    gfe2[ctr] = GaloisFieldElement(&gf, b_fin[i]);
    ctr++;
  }

  GaloisFieldPolynomial poly1(&gf, a_index-1, gfe1);
  GaloisFieldPolynomial poly2(&gf, b_index-1, gfe2);
  GaloisFieldPolynomial poly3;
  ...
  while (cont_loop == 1) {
    std::cout << "[1] A(x) + B(x)" << std::endl;
    std::cout << "[2] A(x) - B(x)" << std::endl;
    std::cout << "[3] A(x) x B(x)" << std::endl;
    std::cout << "[4] A(x) / B(x)" << std::endl;
    ...
    if (choice != 5) {
      std::cout << "A(x): " << poly1 << std::endl;
      std::cout << "B(x): " << poly2 << std::endl;
      std::cout << "Answer: " << poly3 << std::endl << std::endl;
    }
  }
